...
bool IsTrue(const ObjectHolder& holder) {
    if (bool(holder)) {
        if (const Number* num = holder.TryAs<Number>()) {
            return num->GetValue();
        }
        if (const String* str = holder.TryAs<String>()) {
            return !str->GetValue().empty();
        }
        if (const Bool* b = holder.TryAs<Bool>()) {
            return b->GetValue();
        }
    }
    return false;
...
void ClassInstance::Print(std::ostream& os, Context& context) {
    if (HasMethod("__str__"s, 0)) {
        const Method* m = class_.GetMethod("__str__"s);
        Closure fields;
...
const Method* Class::GetMethod(const std::string& name) const {
    auto it = find(methods_.begin(), methods_.end(), name);
    if (it != methods_.end()) {
        return &(*it);
    }
    else if (parent_) {
